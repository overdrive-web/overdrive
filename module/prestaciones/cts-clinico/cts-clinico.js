...
function renderTable() {
  ...
  filteredCtsClinicos.forEach(ctsClinico => {
    const fechaCreacion = ctsClinico.fecha && typeof ctsClinico.fecha.toDate === 'function'
      ? ctsClinico.fecha.toDate()
      : ctsClinico.fecha instanceof Date
      ? ctsClinico.fecha
      : null;
    ...
    tr.innerHTML = `
      ...
      <td>${ctsClinico.email || ''}</td>
      <td>${ctsClinico.observacion || ''}</td>
      <td>${fechaDisplay}</td>
      <td>${ctsClinico.user}</td>
    `;
...
async function init() {
  ...
  onAuthStateChanged(auth, async user => {
    if (!user) {
      ...
      window.location.href = 'main.html?error=ocauth';
      return;
    }
    ...
    try {
      ...
      confirmDeleteBtn.addEventListener('click', async () => {
        ...
        try {
          ...
          showSuccessMessage('Contacto clínico eliminado correctamente');
      ...
      exportExcelBtn.addEventListener('click', () => {
        const data = ctsClinicos.map(cts => {
          const fechaCreacion = cts.fecha && typeof cts.fecha.toDate === 'function'
            ? cts.fecha.toDate()
            : cts.fecha instanceof Date
            ? cts.fecha
            : null;
